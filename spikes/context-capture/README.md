...
It then walks the focused element's parents and prints each one's `AXURL`/`AXDocument`/`AXTitle`, which is how a canvas editor's containing web area is identified. `--poll=<n> --poll-interval=<seconds>` re-reads the same element on a timer with wall-clock stamps, so a target that lags behind the keyboard — or, like Google Docs, never carries the document at all — shows up as a measured number rather than an impression.
