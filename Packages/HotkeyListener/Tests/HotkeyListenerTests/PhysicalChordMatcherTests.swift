...
struct PhysicalChordMatcherTests {
  // MARK: Internal

  @Test func `modifier state uses device specific event flags`() {
    ...
    for (modifier, flags) in downFlags {
      #expect(modifier.isDown(in: flags))
    }
  ...
  @Test func `modifier chord can be built one modifier at A time`() throws {
  ...
  @Test func `keyed chord suppresses only its primary key pair`() throws {
  ...
  @Test func `keyed chord does not suppress primary key with wrong modifiers`() throws {
    let hotkey = try Hotkey(keyCode: 0, modifiers: [.leftCommand])
    var matcher = PhysicalChordMatcher(hotkey: hotkey)
  ...
  @Test func `keyed chord releases when primary key lifts after modifier`() throws {
    let hotkey = try Hotkey(keyCode: 0, modifiers: [.leftCommand])
    var matcher = PhysicalChordMatcher(hotkey: hotkey)
    _ = matcher.receive(
      transition(.modifier(.leftCommand), .down, pressedAfter: [.modifier(.leftCommand)]),
  ...
  @Test func `interruption preserves suppression until paired key up`() throws {
    let hotkey = try Hotkey(keyCode: 0, modifiers: [.leftCommand])
    var matcher = PhysicalChordMatcher(hotkey: hotkey)
    _ = matcher.receive(
      transition(.keyCode(0), .down, pressedAfter: [.modifier(.leftCommand), .keyCode(0)]),
