...
- [x] Phase 1: Hotkey pipeline
  - Empirical TCC findings: the session tap runs on the Accessibility grant (`kTCCServiceAccessibility`) alone — this was originally built against Input Monitoring (`kTCCServiceListenEvent`) on the usual guidance that a listen-only tap needs it, and that turned out to be wrong (see the Phase 7 findings). Tap creation succeeds without any grant, so creation and the listener's `monitoringStarted` signal prove nothing; only `AXIsProcessTrusted` does, and it tracks grants and revocations live in-process. A missing grant is reported as a distinct permission-missing state rather than a silent dead tap. Implementation note: modifier direction must come from the event's own device-side flag bits — `CGEventSource.keyState` lags one event at a head-insert tap.
