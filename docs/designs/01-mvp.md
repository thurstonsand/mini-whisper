...
- [x] Phase 4: Engine + silence gate
  - Empirical findings: FluidAudio v0.15.5's `VadManager` fixes inference at 4096 samples (design amended — see Decision Point 2) and its downloader fetches mutable HF `main`, so the app owns exact-revision download/validation/provenance; `VadSegmentationConfig`'s `negativeThreshold` initializer argument trips a contradictory debug assertion and must be assigned post-init. Post-phase latency work: activation → pill 3–8 ms, activation → first audio buffer ~230 ms (budgeted in `mise run benchmark-live`).
...
- [x] Phase 5: Delivery + sounds
  - Empirical TCC findings: synthetic ⌘V is gated by `kTCCServicePostEvent` (surfaced under Accessibility in System Settings), preflighted via `CGPreflightPostEventAccess` — distinct from `kTCCServiceAccessibility`, so `reset-permissions` resets it separately; a fresh run prompts for all three permissions in gesture order (Input Monitoring at launch, Microphone at first capture, PostEvent at first paste) and each grant requires an app relaunch to take effect. Caveat for onboarding tests: terminal-launched runs can inherit the terminal's Accessibility-implied Input Monitoring via responsible-process attribution — first-run evidence only counts from a Finder-launched .app.
