...
- [x] Phase 1: Hotkey pipeline
  - Empirical TCC findings: the session tap requires Input Monitoring (`kTCCServiceListenEvent`) — creation without it succeeds but keyboard events are silently withheld, so the listener reports a distinct permission-missing state. `IOHIDCheckAccess` / `IOHIDRequestAccess(kIOHIDRequestTypeListenEvent)` own the Input Monitoring pane: the request raises macOS's Keystroke Receiving dialog and seeds the app's row; `CGRequestListenEventAccess` reached TCC but did neither on the validation host. Accessibility preflight must wait until after the Input Monitoring request or it suppresses the IOHID dialog. `IOHIDCheckAccess` remains stale if the human enables the pane but chooses Later at macOS's quit-and-reopen prompt; a keyboard-only probe tap and the listener's `monitoringStarted` signal are false positives because tap creation succeeds before authorization. Onboarding therefore tells the human to quit and reopen, after which the launch-time status is correct and derived state resumes the flow. Implementation note: modifier direction must come from the event's own device-side flag bits — `CGEventSource.keyState` lags one event at a head-insert tap.
...
- [x] Phase 7: Onboarding
  - Empirical TCC findings (macOS 26.5.2): tccd refuses to prompt for Input Monitoring at all — `Service kTCCServiceListenEvent does not allow prompting; returning denied.` — so `IOHIDRequestAccess`, `CGRequestListenEventAccess`, and a listen-only event-tap attempt each come back denied with no dialog and, worse, no Input Monitoring row for the user to switch on. Running from `/Applications`, registering with Launch Services, and holding Accessibility first change nothing; Apple's Tahoe documentation describes only user-managed rows, and the developer forums corroborate (thread 828082). The pane's + button is the supported path, so the permissions step now spells the manual add out — deep link, this build's bundle path, ⇧⌘G — and keeps Restart MiniWhisper for after the add. A `false` return cannot distinguish Tahoe's silent refusal from a pre-26 dialog still on screen, so the guidance is state-driven (the request finished and left no grant) rather than version-gated. Evidence: `/tmp/imtest2.tcc.log` through `/tmp/imtest6.tcc.log`.
  - Goal: A fresh machine reaches "Ready" through one guided flow.
  - Files: Onboarding feature + window in `MiniWhisper/`.
  - Work: First-run detection; one startup join resolves completion, download consent, IOHID, microphone, conditionally safe Accessibility status, and the engine's first readiness value into the initial snapshot. A one-time welcome requests explicit consent before writing a durable model-download marker and starting the ~450 MB transfer in the background. A consented relaunch skips the welcome and resumes a pin-matched staging tree's manifest-verified completed files plus any SHA-covered ranged partial from `Engine/pinned-v1.partial`; every file is hashed before atomic promotion, while ordinary launch validation is existence/size. The permissions page polls once a second but exposes exactly one action at a time in Input Monitoring → Microphone → Paste Access order; each native request is issued once on the main actor, and status polling is suspended until MiniWhisper becomes active again because the IOHID and Accessibility request APIs return while their dialogs are still pending. Accessibility is never probed before the IOHID request or an existing Input Monitoring grant. Input Monitoring's stale in-process status offers Restart MiniWhisper after the initial request, alongside manual-add guidance carrying the running bundle's path, because macOS 26 never prompts; this uses a new application instance and terminates the stale one, while macOS's own Quit & Reopen remains the fast path. Derived state resumes setup after the reopen; the rail carries compact background model progress and the full model page continues through compile/prewarm (the 213 s cost lives here, visibly); terminal "try it" moment with a completion-marking Skip escape hatch; degraded menu items re-enter the relevant step.
