...
- **A page that opens focused says so once, declaratively.** `.defaultFocus($focus, .target, priority: .userInitiated)` is the whole cure: the priority is what outranks AppKit's own choice of key view. Chasing the same race with `onAppear`, a `didBecomeKey` hook, and a yield-then-focus task stacks three assertions of one rule and still loses — honest paint on dishonest focus is what a stray ring on entry actually is.
