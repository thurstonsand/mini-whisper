import HotkeyListener

extension Hotkey {
  var displayComponents: [String] {
    HotkeyDisplayName.components(keyCodes: keyCode.map { [$0] } ?? [], modifiers: modifiers)
  }

  var displayName: String {
    displayComponents.joined(separator: " ")
  }
}

extension HotkeyRecordingChord {
  var displayComponents: [String] {
    HotkeyDisplayName.components(keyCodes: keyCodes.sorted(), modifiers: modifiers)
  }

  var displayName: String {
    displayComponents.joined(separator: " ")
  }
}

// MARK: - HotkeyDisplayName

enum HotkeyDisplayName {
  // MARK: Internal

  static func components(keyCodes: [UInt16], modifiers: Set<ModifierKey>) -> [String] {
    modifiers.sorted { modifierRank($0) < modifierRank($1) }.map(modifierName)
      + keyCodes.map(keyName)
  }

  static func keyName(_ keyCode: UInt16) -> String {
    keyNames[keyCode] ?? "Key \(keyCode)"
  }

  // MARK: Private

  private static let keyNames: [UInt16: String] = [
    0: "A", 1: "S", 2: "D", 3: "F", 4: "H", 5: "G", 6: "Z", 7: "X", 8: "C",
    9: "V", 11: "B", 12: "Q", 13: "W", 14: "E", 15: "R", 16: "Y", 17: "T",
    18: "1", 19: "2", 20: "3", 21: "4", 22: "6", 23: "5", 24: "=", 25: "9",
    26: "7", 27: "-", 28: "8", 29: "0", 30: "]", 31: "O", 32: "U", 33: "[",
    34: "I", 35: "P", 36: "Return", 37: "L", 38: "J", 39: "'", 40: "K", 41: ";",
    42: "\\", 43: ",", 44: "/", 45: "N", 46: "M", 47: ".", 48: "Tab", 49: "Space",
    50: "`", 51: "Delete", 71: "Clear", 76: "Enter", 96: "F5", 97: "F6", 98: "F7",
    99: "F3", 100: "F8", 101: "F9", 103: "F11", 105: "F13", 106: "F16", 107: "F14",
    109: "F10", 111: "F12", 113: "F15", 114: "Help", 115: "Home", 116: "Page Up",
    117: "Forward Delete", 118: "F4", 119: "End", 120: "F2", 121: "Page Down", 122: "F1",
    123: "←", 124: "→", 125: "↓", 126: "↑",
  ]

  private static func modifierRank(_ modifier: ModifierKey) -> Int {
    switch modifier {
    case .leftControl,
         .rightControl:
      0
    case .leftOption,
         .rightOption:
      1
    case .leftShift,
         .rightShift:
      2
    case .leftCommand,
         .rightCommand:
      3
    case .function:
      4
    }
  }

  private static func modifierName(_ modifier: ModifierKey) -> String {
    switch modifier {
    case .leftControl:
      "⌃ Ctrl ←"
    case .rightControl:
      "⌃ Ctrl →"
    case .leftOption:
      "⌥ Opt ←"
    case .rightOption:
      "⌥ Opt →"
    case .leftShift:
      "⇧ Shift ←"
    case .rightShift:
      "⇧ Shift →"
    case .leftCommand:
      "⌘ Cmd ←"
    case .rightCommand:
      "⌘ Cmd →"
    case .function:
      "fn"
    }
  }
}
